...
## (Agradecimientos a Franco Borrelli por su contribución con la teoría).
...
1. El framework está compuesto de diferentes librerías:
   ...
   * ActionCable
   ...
   - ActionMailer: permite enviar mails desde nuestra aplicación usando clases y vistas. Trabaja de manera similar a los controladores.
   http://guides.rubyonrails.org/action_mailer_basics.html

   - ActionPack: es un framework que nos provee el ruteo de nuestra aplicación web. Utiliza controladores para definir las acciones ante una determinada ruta, y vistas para renderizar.

   - ActionView: nos permite realizar el manejo de vistas. No nos obliga a usar una sintaxis específica para el template. Nos da lógica para generar links, por ejemplo.

   - ActiveJob: es un framework utilizado para crear tareas en paralelo. Abstrae el manejo de tareas de segundo plano. Relacionada a los controladores.

   - ActiveRecord: es un framework ORM. Mapea tablas relacionadas en la base de datos a clases, para poder acceder a tuplas como si fueran objetos. Relacionada al modelo.

   - ActiveSupport: es un conjunto de extensiones que brindan facilidades a Rails, ya sea para manejo de caché, entre otras muchas cosas. Las librerías generalmente favorecen al controlador, pero podrían tener extensiones en la vista como en el modelo. Es la mas controversial. Es un gran monkey path. Por ejemplo: se incluye el módulo de fechas que permite transformar enteros en días (tiempo en minutos). Agrega también nuevas clases.

   - ActionCable: Manejo de websockets. Participa tanto del lado del cliente como del servidor.

   - ActiveStorage: abstraccion de almacenamiento en la nuve. Podemos abstraernos de la subida de archivos.
...
2. Investigá cómo se crea una aplicación Rails nueva y enumerá los pasos básicos para tener la aplicación funcionando con una
   base de datos SQLite.
    ...
    Para crear una aplicación, se usa el comando: ` rails new nombre_aplicación `. Esto nos va a crear una jerarquía de proyectos, e instalar las gemas de las cuales depende rails. Es posible requerir mas, o menos gemas.

    Por defecto Rails configura para que funcione con una base de datos SQLite.

    Esto se puede cambiar con la opción ` -d ` o ` --database `. Los parámetros aceptados son: mysql, oracle, postgresql, sqlserver, entre otros.
